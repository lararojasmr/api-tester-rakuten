...
class TVideoQualitiesPut(VideoQualities):

    def test_video_qualities_not_change_with_existing_name(self, api_ops, json_schema_validator):
        # Precondition: We have and item add in the system with default
        quality_name = "The Chum's Quality " + str(round(time.time() * 1000))
        quality_one = api_ops.post(url=TVideoQualitiesPut.path, json={
            "name": quality_name,
            "abbr": "TCQ " + str(round(time.time() * 1000))
        })
        assert quality_one.status_code == 201
        quality_one_obj = json.loads(quality_one.content)
        quality_one = api_ops.get(TVideoQualitiesPut.path + '/' + str(quality_one_obj['id']))
        assert quality_one.status_code == 200

        quality_two_abbr = "TCQ " + str(round(time.time() * 1000))
        quality_two = api_ops.post(url=TVideoQualitiesPut.path, json={
        ...
        quality_two = api_ops.get(TVideoQualitiesPut.path + '/' + str(quality_two_obj['id']))
        assert quality_two.status_code == 200

        # Try to update with and existing test case
        quality_two_edited = api_ops.put(url=TVideoQualitiesPut.path, json={
            "name": quality_name,
            "abbr": quality_two_abbr
        })
    ...
    def test_video_qualities_not_change_with_existing_abbr(self, api_ops, json_schema_validator):
        # Precondition: We have and item add in the system with default
    ...
    def test_video_qualities_not_change_without_name(self, api_ops, json_schema_validator):
        # Precondition: We have and item add in the system with default
        quality_abbr = "TCQ " + str(round(time.time() * 1000))
        quality_one = api_ops.post(url=TVideoQualitiesPut.path, json={
            "name": "The Chum's Quality " + str(round(time.time() * 1000)),
        ...
        quality_one_edited = api_ops.put(url=TVideoQualitiesPut.path + '/' + str(quality_one_obj['id']), json={
            "name": None,
            "abbr": quality_abbr
        })
    ...
    def test_video_qualities_not_change_without_abbr(self, api_ops, json_schema_validator):
        # Precondition: We have and item add in the system with default
        quality_name = "The Chum's Quality " + str(round(time.time() * 1000))
        quality_one = api_ops.post(url=TVideoQualitiesPut.path, json={
            "name": quality_name,
    ...
    def test_video_qualities_change_record_parameters(self, api_ops, json_schema_validator):
        # Precondition: We have and item add in the system with default
        ...
        quality_one_edited = api_ops.get(TVideoQualitiesPut.path + '/' + str(quality_one_obj['id']))
